...
if (instance.value.headSha !== authority.value.baseSha || instance.value.targetRemote !== 'origin' || instance.value.model !== 'gpt-6-astra' || instance.value.reasoningEffort !== 'xhigh') fail('E_RUNTIME_OR_IDENTITY');
...
process.stdout.write(`${JSON.stringify({schemaVersion:'STAGE_ADMISSION_VERIFIER_V3_RESULT',status:'PASS',authorityDigest:authority.digest,stageInstanceDigest:instance.digest,stageAdmissionDigest:admission.digest,writeSetDigest,commandScopeDigest,acceptanceSignalsDigest,verifierDigest:selfDigest,repoRoot})}\n`);
