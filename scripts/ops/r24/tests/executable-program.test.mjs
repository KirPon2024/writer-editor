...
test('PlanState persists the full 109-node denominator without unreconciled DONE overclaim', () => {
  ...
  assert.equal(state.transitionHistory.length, 25);
  assert.deepEqual(
...
test('scheduler selection receipt is bound to the real full graph rather than a fixture', () => {
  ...
  assert.equal(receipt.selectedId, 'R5_LIFECYCLE_EXTERNAL_CONFLICT');
  ...
  assert.deepEqual(receipt.readySet, ['R5_LIFECYCLE_EXTERNAL_CONFLICT']);
});
