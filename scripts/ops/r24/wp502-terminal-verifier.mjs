...
const PATHS = Object.freeze({
  authority: `${C}/WP502_MAIN_PRODUCT_OWNER_AUTHORITY_V1.json`,
  selection: `${C}/WP502_MAIN_PRODUCT_SELECTION_RECEIPT_V1.json`,
  instance: `${C}/WP502_MAIN_PRODUCT_STAGE_INSTANCE_V1.json`,
  admission: `${C}/WP502_MAIN_PRODUCT_STAGE_ADMISSION_ATTESTATION_V1.json`,
...
export function verifyWp502TerminalCarriers() {
  ...
  assert(admission.digest === selection.value.admissionBindings.stageAdmissionDigest, 'E_WP502_ADMISSION_DIGEST');
  ...
  assert(matrix.value.status === 'CONDITIONAL_PASS_PENDING_EXTERNAL_DELIVERY' && matrix.value.localPassedRowCount === 16 && matrix.value.externalPredicateRowCount === 4 && matrix.value.failedRowCount === 0 && matrix.value.pendingRowCount === 0, 'E_WP502_MATRIX_STATE');
  assert(matrix.value.rows.length === 20 && matrix.value.rows.filter((row) => row.status === 'PASS').length === 16 && matrix.value.rows.filter((row) => row.status === 'REQUIRED_EXTERNAL_PREDICATE').length === 4, 'E_WP502_MATRIX_DENOMINATOR');
  ...
  return {
    ...
    evidenceStampDenominator: evidenceFiles.length, localPassedRows: 16, externalPredicateRows: 4,
