...
test('post-evaluation bytes require the exact chained audit-cycle-two WP401 WP402 WP403 WP404 WP500 WP501 WP502 and WP503 admissions',()=>{
  ...
  assert.equal(wp503.admittedPathDenominator,86);
  assert.equal(wp503.predecessor.admittedPathDenominator,76);
...
test('WP503 temporal successor exception rejects an unadmitted future path',()=>{
  ...
  assert.throws(()=>verifyWp503MainProductPostEvaluationException({candidateSha:'HEAD',git:hostileGit}),/E_WP503_TEMPORAL_UNADMITTED_PATH:README\.md/);
});
