...
- Не меняем формат данных без отдельного ТЗ (см. `docs/BIBLE.md`)
- Не добавляем зависимости/библиотеки без явного разрешения
- Не добавляем сетевые функции/аккаунты/облако
- Не меняем дизайн/структуру UI без отдельного ТЗ
...
### Данные/хранение
- Не делаем DOM/HTML источником истины (surface ≠ core).
- Бэкапы/автосейв/атомарная запись/Recovery — по канону `docs/BIBLE.md`.
...
- `Cmd/Ctrl+C/V/X/A` работают ожидаемо (и в редакторе, и в полях меты)
