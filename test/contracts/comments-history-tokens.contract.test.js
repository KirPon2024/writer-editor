...
test('comments-history safe state script emits authoritative token', () => {
  const state = spawnSync(process.execPath, ['scripts/ops/comments-history-safe-state.mjs', '--json'], {
    encoding: 'utf8',
  });
  assert.equal(state.status, 0, `comments-history-safe-state failed:\n${state.stdout}\n${state.stderr}`);
  const payload = JSON.parse(String(state.stdout || '{}'));
  assert.equal(payload.COMMENTS_HISTORY_SAFE_OK, 1);
});

test('comments-history safe token is exposed by truth-table, ops-summary and doctor', () => {
  ...
  assert.equal(truthTable.COMMENTS_HISTORY_SAFE_OK, 1);
  ...
  assert.equal(summaryTokens.get('OPS_SUMMARY_COMMENTS_HISTORY_SAFE_OK'), '1');
  ...
  assert.equal(doctorTokens.get('COMMENTS_HISTORY_SAFE_OK'), '1');
});
