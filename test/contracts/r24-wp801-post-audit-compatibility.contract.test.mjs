...
test('WP801 candidate oracle binds the exact 41-path admission and protected baseline', () => {
  const result = verifyWp801MainProductPostEvaluationException({ git: fakeGit() });
...
test('WP801 candidate oracle rejects scope, base, ancestry, missing-artifact and byte drift', () => {
  assert.throws(() => verifyWp801MainProductPostEvaluationException({ git: fakeGit({ changedPaths: [...ADMITTED, 'src/main.js'].sort() }) }), /E_WP801_EXACT_ADMITTED_DELTA/u);
  assert.throws(() => verifyWp801MainProductPostEvaluationException({ git: fakeGit({ changedPaths: ADMITTED.slice(1) }) }), /E_WP801_EXACT_ADMITTED_DELTA/u);
  assert.throws(() => verifyWp801MainProductPostEvaluationException({ git: fakeGit({ baseTreeDrift: true }) }), /E_WP801_ADMISSION_BASE/u);
  assert.throws(() => verifyWp801MainProductPostEvaluationException({ git: fakeGit({ ancestor: false }) }), /E_WP801_BASE_NOT_ANCESTOR/u);
  assert.throws(() => verifyWp801MainProductPostEvaluationException({ git: fakeGit({ missingArtifact: E.instancePath }) }), /E_WP801_CANDIDATE_ARTIFACT_MISSING/u);
  assert.throws(() => verifyWp801MainProductPostEvaluationException({ git: fakeGit({ byteDrift: E.admissionPath }) }), /E_WP801_CANONICAL_LF/u);
});

test('WP801 candidate oracle rejects a forged lease and carrier-registry fallback', () => {
  assert.throws(() => verifyWp801MainProductPostEvaluationException({ git: fakeGit({ mutateJson: { path: E.instancePath, apply: (value) => { value.lease.wip = 0; } } }) }), /E_WP801_ADMISSION_CARRIER_DIGEST/u);
  assert.throws(() => verifyWp801MainProductPostEvaluationException({ git: fakeGit({ mutateJson: { path: 'docs/OPS/R24/CORRECTIVE/WP801_CARRIER_REGISTRY_V1.json', apply: (value) => { value.currentTreeFallbackAllowed = true; } } }) }), /E_WP801_CARRIER_DENOMINATOR/u);
});
