...
const PATHS = Object.freeze({
  ...
  carrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V16.json`,
  predecessorCarrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V15.json`,
...
export function checkCorrections(root = process.cwd()) {
  ...
  assert(predecessorCarrierRegistry.digest === '8b3cb7045ca45e1c98928e105dd4558afb14d33650ba667a2ad6ffe8773ac87f' && predecessorCarrierRegistry.value.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V15', 'E_CARRIER_REGISTRY_PREDECESSOR_DIGEST', predecessorCarrierRegistry.digest);
  ...
  assert(carrierRegistry.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V16' && carrierRegistry.baseSha === stage.value.baseSha && carrierRegistry.baseTreeSha === stage.value.treeSha, 'E_CARRIER_REGISTRY_BINDING', carrierRegistry.schemaVersion);
  assertExactJson(carrierRegistry.predecessor,{path:PATHS.predecessorCarrierRegistry,sha256:predecessorCarrierRegistry.digest,status:'SUPERSEDED_BY_APPEND_ONLY_WP503_POST_AUDIT_CERTIFICATION_CHAIN_SUCCESSOR'},'E_CARRIER_REGISTRY_PREDECESSOR','binding');
  ...
  assertExactJson(carrierRegistry.successorScope,{reason:'CURRENT_REPOSITORY_POST_AUDIT_CERTIFICATION_CHAIN_REFRESHED_BY_ADMITTED_WP503_APPEND_ONLY_SUCCESSOR',replacementPaths:['.github/workflows/oss-policy.yml',`${ROOT}/C1B_TEST_INVENTORY_V1.json`,'scripts/ops/r24/corrective/audit-r2-corrections.mjs','scripts/ops/r24/corrective/post-audit-certification-set.mjs','scripts/ops/r24/wp503-terminal-verifier.mjs','test/contracts/r24-audit-r2-corrections.contract.test.mjs','test/contracts/r24-post-audit-certification-set.contract.test.mjs','test/contracts/r24-wp503-terminal-carriers.contract.test.mjs'],addedPaths:[`${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V16.json`,`${ROOT}/WP503_GOVERNANCE_CHANGE_APPROVALS_V3.json`,`${ROOT}/WP503_LOCAL_POST_AUDIT_FAILURE_V1.json`,`${ROOT}/WP503_MAIN_PRODUCT_OWNER_AUTHORITY_AMENDMENT_V6.json`,`${ROOT}/WP503_MAIN_PRODUCT_STAGE_ADMISSION_ATTESTATION_V6.json`,`${ROOT}/WP503_MAIN_PRODUCT_STAGE_INSTANCE_V6.json`,`${ROOT}/WP503_POST_AUDIT_CERTIFICATION_SUCCESSOR_V1.json`,`${ROOT}/WP503_TERMINAL_SUPPLEMENT_V2.json`],removedPaths:[]},'E_CARRIER_REGISTRY_SUCCESSOR_SCOPE','scope');
