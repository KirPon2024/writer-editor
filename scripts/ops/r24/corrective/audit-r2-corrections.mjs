...
const PATHS = Object.freeze({
  ...
  carrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V9.json`,
  predecessorCarrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V8.json`,
...
export function checkCorrections(root = process.cwd()) {
  ...
  assert(predecessorCarrierRegistry.digest === 'a019163ce23c514ed88e1565843827c4928486f6efde6709fc3f5ae485433c01' && predecessorCarrierRegistry.value.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V8', 'E_CARRIER_REGISTRY_PREDECESSOR_DIGEST', predecessorCarrierRegistry.digest);
  ...
  assert(carrierRegistry.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V9' && carrierRegistry.baseSha === stage.value.baseSha && carrierRegistry.baseTreeSha === stage.value.treeSha, 'E_CARRIER_REGISTRY_BINDING', carrierRegistry.schemaVersion);
  assertExactJson(carrierRegistry.predecessor,{path:PATHS.predecessorCarrierRegistry,sha256:predecessorCarrierRegistry.digest,status:'SUPERSEDED_BY_APPEND_ONLY_WP501_TERMINAL_EXCEPTION_TEST_INVENTORY_SUCCESSOR'},'E_CARRIER_REGISTRY_PREDECESSOR','binding');
