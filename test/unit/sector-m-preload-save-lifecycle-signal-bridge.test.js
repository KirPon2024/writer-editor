...
test('preload save lifecycle signal bridge: main exposes one handler with strict signal allowlist', () => {
  ...
  assert.ok(source.includes('if (payload.state !== true) {'))
  assert.ok(source.includes("return { ok: true, ignored: true, reason: 'RENDERER_FALSE_CANNOT_CLEAR_MAIN_DIRTY' };"))
  assert.ok(source.includes('lastSignaledEditGeneration = mergeSignaledGeneration(lastSignaledEditGeneration, payload.generation);'))
  assert.ok(source.includes('isDirty = true;'))
