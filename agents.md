...
2) `docs/BIBLE.md` — канон проекта и дорожная карта.
3) `README.md` — короткое описание и ссылки.
4) `docs/CONTEXT.md` — текущее состояние, инварианты, ближайшие шаги.
5) `docs/PROCESS.md` — как работаем (ChatGPT → Codex, diff‑budget, проверки).
6) `docs/HANDOFF.md` — последний срез для быстрого входа.

Если `CANON.md`/`docs/BIBLE.md` не дают точного ответа — задать 1–3 уточняющих вопроса (только high‑impact) и остановиться до ответа.
