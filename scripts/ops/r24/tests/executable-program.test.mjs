...
test('PlanState persists the full 109-node denominator without unreconciled DONE overclaim', () => {
  ...
  assert.equal(state.transitionHistory.length, 35);
  assert.deepEqual(
...
test('scheduler selection receipt is bound to the real full graph rather than a fixture', () => {
  ...
  assert.equal(receipt.selectedId, 'F0_WRITER_REFINEMENT_CONFORMANCE');
  ...
  assert.deepEqual(receipt.readySet, ['F0_WRITER_REFINEMENT_CONFORMANCE']);
});
