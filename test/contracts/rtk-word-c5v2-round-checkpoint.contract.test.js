...
test('C5V2 physical Word chunks preserve root-first and descending-range authority with cumulative readback counts', async () => {
  ...
  assert.match(continuation, /set oldUserName to user name as text/u);
  assert.match(continuation, /set oldUserInitials to user initials as text/u);
  assert.match(continuation, /set user name to \(oldUserName as text\)/u);
  assert.match(continuation, /set user initials to \(oldUserInitials as text\)/u);
  assert.match(continuation, /on error yCanaryErrMsg number yCanaryErrNo/u);
  assert.match(continuation, /set yCanaryErrText to yCanaryErrMsg as text/u);
  ...
  assert.match(continuation, /READBACK\|insert-mid\|BLOCKED\|" & \(yNativeReadbackErrNo as text\) & ":" & \(yNativeReadbackErrMsg as text\)/u);
  assert.doesNotMatch(continuation, /on error errMsg number errNo/u);
