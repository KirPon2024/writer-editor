...
export function evaluateWordV4E12SaturationLedger(input = {}) {
  ...
  if (receipt.status !== 'WORD_SATURATION_LEDGER_BOUND_NOT_SATURATED') {
    add('RTK_V4_E12_STATUS_INVALID', 'status', 'E12 must stay not-saturated until wave criteria are proven.');
  }
  ...
  if (wave.completedWaves?.includes(10) !== true || Number(wave.currentWaveTarget) !== 40 || Number(wave.currentWaveObservedRounds) !== 32) {
    add('RTK_V4_E12_WAVE_ACCOUNTING_INVALID', 'saturationRule', 'E12 must bind the completed wave 10 and the incomplete 40-round wave with 32 observed rounds.');
  }
  ...
  for (const required of ['unicodeAndBidi', 'hostilePackage', 'performanceScale', 'crashRecovery', 'replayIdempotence']) {
  ...
  if (!cell || cell.state !== 'PHYSICAL_WORD_PROVEN' || cell.currentCapability !== 'SATURATION_LEDGER_BOUND_NOT_SATURATED' || cell.physicalWordEvidence !== true) {
    add('RTK_V4_E12_PROFILE_CELL_INVALID', 'profile.cells.rtk.word.v4.saturationLedger', 'Capability profile must bind E12 as physical evidence proven but not saturated.');
  }
  if (profile.status !== 'WORD_16_111_2_E12_SATURATION_LEDGER_BOUND_NOT_SATURATED') {
    add('RTK_V4_E12_PROFILE_STATUS_INVALID', 'profile.status', 'Profile status must reflect E12 not-saturated ledger.');
  }

  const state = program.v4ExecutionState || {};
  if (program.status !== 'WORD_E12_SATURATION_LEDGER_BOUND_NOT_SATURATED') {
    add('RTK_V4_E12_PROGRAM_STATUS_INVALID', 'program.status', 'Program status must reflect E12 not-saturated ledger.');
  }
  if (state.status !== 'EXECUTION_12_LOCAL_VERIFIED_NOT_SATURATED_CONTINUE_WORD_WAVES') {
    add('RTK_V4_E12_PROGRAM_STATE_INVALID', 'program.v4ExecutionState.status', 'Program state must keep E12 active until saturation waves complete.');
  }
  if (state.nextStage !== 'EXECUTION_12_NEXT_PHYSICAL_WAVE_40') {
    add('RTK_V4_E12_NEXT_STAGE_INVALID', 'program.v4ExecutionState.nextStage', 'Next stage must continue the 40-round Word wave, not Google Docs.');
  }
