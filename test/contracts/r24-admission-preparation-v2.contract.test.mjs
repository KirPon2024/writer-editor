...
import {renderAdmissionPath,prepareGeneratedOutputSet,runPreparation} from '../../scripts/ops/r24/corrective/admission-preparation-v2.mjs';
...
function nativeFixture(t){
  ...
  const verifierPath='scripts/ops/r24/corrective/stage-admission-verifier-v3.mjs';
  ...
  Object.assign(authority,{stageId:f.instance.stageId,baseSha:git('rev-parse','HEAD'),baseTree:git('rev-parse','HEAD^{tree}'),branch:'codex/p01-native',verifierDigest:'aa3ee410111bc245b6d5fa2e13864957887bd4634795be839a68d7ed507cd542',fixedBindings:[{path:'anchor.json',sha256:sha(Buffer.from('{}\n'))}],allowedOperations:f.instance.operations,predecessors:f.instance.predecessors,lease:{fencingCounter:83,status:'ACTIVE',wip:1,predecessorReleaseDigest:f.instance.lease.predecessorReleaseDigest}});
  for(const key of ['stageId','baseSha','branch','fixedBindings','predecessors','lease'])instance[key]=authority[key];
  instance.headSha=authority.baseSha;instance.treeSha=authority.baseTree;instance.operations=authority.allowedOperations;instance.model='gpt-6-astra';instance.reasoningEffort='xhigh';
  for(const key of ['stageId','lease'])admission[key]=instance[key];
  admission.authorityDigest=sha(bytes(authority));admission.stageInstanceDigest=sha(bytes(instance));admission.writeSetDigest=f.admission.writeSetDigest;admission.verifierDigest=authority.verifierDigest;
...
test('real CLI runs the pinned native verifier and produces output without repository writes, including symlinked invocation',t=>{
  const f=nativeFixture(t),helper=path.join(root,'scripts/ops/r24/corrective/admission-preparation-v2.mjs'),link=path.join(f.dir,'helper.mjs');
  ...
  assert.equal(output.status,'PASS');assert.equal(output.repositoryWrites,0);assert.equal(output.outputPathDenominator,1);assert.equal(output.nativeVerifierDigest,'aa3ee410111bc245b6d5fa2e13864957887bd4634795be839a68d7ed507cd542');
