...
const PATHS = Object.freeze({
  ...
  carrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V4.json`,
  predecessorCarrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V3.json`,
...
export function checkCorrections(root = process.cwd()) {
  ...
  assert(predecessorCarrierRegistry.digest === '290ad8cd3db9c25e1a3a17b24b405f07655c7fc3640dd27d28d6ddf96987ee00' && predecessorCarrierRegistry.value.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V3', 'E_CARRIER_REGISTRY_PREDECESSOR_DIGEST', predecessorCarrierRegistry.digest);
  ...
  assert(carrierRegistry.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V4' && carrierRegistry.baseSha === stage.value.baseSha && carrierRegistry.baseTreeSha === stage.value.treeSha, 'E_CARRIER_REGISTRY_BINDING', carrierRegistry.schemaVersion);
  assertExactJson(carrierRegistry.predecessor,{path:PATHS.predecessorCarrierRegistry,sha256:predecessorCarrierRegistry.digest,status:'SUPERSEDED_BY_APPEND_ONLY_WP403_TEST_INVENTORY_SUCCESSOR'},'E_CARRIER_REGISTRY_PREDECESSOR','binding');
  ...
  assertExactJson(carrierRegistry.excludedSelfCarriers, [PATHS.carrierRegistry,PATHS.predecessorCarrierRegistry,`${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V2.json`,PATHS.rootCarrierRegistry,`${ROOT}/C1C_GOVERNANCE_CHANGE_APPROVALS_V1.json`], 'E_CARRIER_REGISTRY_EXCLUSIONS', 'excludedSelfCarriers');
