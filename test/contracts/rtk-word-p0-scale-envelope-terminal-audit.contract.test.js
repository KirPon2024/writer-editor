...
test('P0 scale envelope binds profile program ledger and normalized matrix terminal state', async () => {
  ...
  assert.equal(result.status, 'PASS', JSON.stringify(result.issues, null, 2));
  assert.equal(profile.status, 'WORD_NORMALIZED_CAPABILITY_MATRIX_SUPPORT_ENVELOPE_READY_FOR_INDEPENDENT_AUDIT');
  assert.equal(saturationCell.currentCapability, 'SUPPORT_ENVELOPE_TERMINAL_READY_FOR_INDEPENDENT_AUDIT');
  assert.equal(program.status, 'WORD_NORMALIZED_CAPABILITY_MATRIX_SUPPORT_ENVELOPE_READY_FOR_INDEPENDENT_AUDIT');
  assert.equal(program.nextStep, 'READY_FOR_FRESH_INDEPENDENT_EXACT_HEAD_AUDIT');
  ...
  assert.equal(ledger.status, 'WORD_NORMALIZED_CAPABILITY_MATRIX_SUPPORT_ENVELOPE_READY_FOR_INDEPENDENT_AUDIT');
  ...
  assert.deepEqual(ledger.notSaturatedReasons, []);
  assert.equal(matrix.status, 'WORD_NORMALIZED_CAPABILITY_MATRIX_SUPPORT_ENVELOPE_READY_FOR_INDEPENDENT_AUDIT');
  assert.equal(matrix.counts.blocksWordSaturation, 0);
  assert.equal(matrixSaturationRow.blocksWordSaturation, false);
  assert.equal(matrix.supportEnvelope.readyForFreshIndependentExactHeadAudit, true);
});
