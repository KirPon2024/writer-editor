...
test('PlanState persists the full 109-node denominator without unreconciled DONE overclaim', () => {
  ...
  assert.equal(state.transitionHistory.length, 130);
  assert.deepEqual(
...
test('scheduler selection receipt is bound to the real full graph rather than a fixture', () => {
  ...
  assert.equal(receipt.selectedId, 'WP-307_WRITER_LOCAL_PROFILE');
  ...
  assert.deepEqual(receipt.readySet, ['WP-307_WRITER_LOCAL_PROFILE']);
});
