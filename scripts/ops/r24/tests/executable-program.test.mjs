...
test('PlanState persists the full 109-node denominator without unreconciled DONE overclaim', () => {
  ...
  assert.equal(state.transitionHistory.length, 30);
  assert.deepEqual(
...
test('scheduler selection receipt is bound to the real full graph rather than a fixture', () => {
  ...
  assert.equal(receipt.selectedId, 'R6_MIGRATION_HISTORY_BACKUP_GC');
  ...
  assert.deepEqual(receipt.readySet, ['R6_MIGRATION_HISTORY_BACKUP_GC']);
});
