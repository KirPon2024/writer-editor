...
export const WP505_MAIN_PRODUCT_ADMISSION_EXPECTATIONS=Object.freeze([
  ...
  Object.freeze({
    ...
    writeSetDigest:'5ab8d9247b0dc4c063be78a4b9deae6682fa38542f11b43b13ed6bb6bee4e79c',baseSha:'e42e92ba73168b9b8105651729752eec7d2b65ad',baseTree:'fa08a7571bce5f5ea5e90a32c0d57a9915e130a5',leaseCounter:70,pathDenominator:10
  })
]);
...
export function verifyWp505MainProductPostEvaluationException({candidateSha='HEAD',git=defaultGit}={}){
  ...
  const [v1,v2]=verified,resolvedCandidate=gitText(git,['rev-parse',candidateSha]);
  assert(evaluationTree(git,v1.candidateSha)===v1.candidateTree,'E_WP505_V1_CANDIDATE_TREE');
  try{git(['merge-base','--is-ancestor',v1.baseSha,v1.candidateSha],{encoding:null});}catch{fail('E_WP505_V1_BASE_NOT_ANCESTOR');}
  try{git(['merge-base','--is-ancestor',v1.candidateSha,resolvedCandidate],{encoding:null});}catch{fail('E_WP505_V2_CANDIDATE_NOT_ANCESTOR');}
  ...
  const v2Changed=gitText(git,['diff','--name-only',`${v2.baseSha}..${resolvedCandidate}`]).split('\n').filter(Boolean).sort();
  for(const changedPath of v2Changed)assert(v2.admittedPaths.includes(changedPath),'E_WP505_V2_UNADMITTED_PATH',changedPath);
  const failure=readJsonFile(v2.failurePath),successor=readJsonFile(v2.successorPath);
  assert(failure.digest===v2.failureDigest&&failure.value.schemaVersion==='YALKEN_R24_WP505_CANDIDATE_CI_FAILURE_V1'&&failure.value.status==='FAIL_CLOSED_SUPERSEDED_BY_APPEND_ONLY_POST_AUDIT_COMPATIBILITY_SUCCESSOR'&&failure.value.exactCandidate?.sha===v1.candidateSha&&failure.value.exactCandidate.tree===v1.candidateTree&&failure.value.exactCandidate.runId===33584273368&&failure.value.exactCandidate.attempt===1&&failure.value.exactCandidate.conclusion==='FAILURE','E_WP505_CANDIDATE_FAILURE_BINDING');
  assert(failure.value.causalFailures?.length===6&&failure.value.derivedFailures?.length===2&&failure.value.successfulRequiredEvidence?.rtkRequiredStatus==='SUCCESS'&&failure.value.successfulRequiredEvidence.liveRulesetOracleStatus==='SUCCESS'&&failure.value.lease?.fencingCounter===70&&failure.value.lease.status==='ACTIVE'&&failure.value.lease.wip===1&&failure.value.programDone===false,'E_WP505_CANDIDATE_FAILURE_DENOMINATOR');
  assert(successor.value.schemaVersion==='YALKEN_R24_WP505_POST_AUDIT_COMPATIBILITY_SUCCESSOR_V1'&&successor.value.status==='CURRENT_APPEND_ONLY_SUCCESSOR'&&successor.value.stageId==='WP-505_REGISTER_AND_ASK','E_WP505_SUCCESSOR_SCHEMA');
  assert(successor.value.admission?.authorityDigest===v2.authorityDigest&&successor.value.admission.stageInstanceDigest===v2.instanceDigest&&successor.value.admission.stageAdmissionDigest===v2.admissionDigest&&successor.value.admission.writeSetDigest===v2.writeSetDigest&&successor.value.bindings?.failureDigest===failure.digest&&successor.value.bindings.wp505V1AuthorityDigest===v1.authorityDigest&&successor.value.bindings.wp505V1StageInstanceDigest===v1.instanceDigest&&successor.value.bindings.wp505V1StageAdmissionDigest===v1.admissionDigest,'E_WP505_SUCCESSOR_ADMISSION_BINDING');
  for(const [pathKey,artifactPath] of [['postAuditCertificationVerifierDigest','scripts/ops/r24/corrective/post-audit-certification-set.mjs'],['requiredWorkflowDigest','.github/workflows/oss-policy.yml'],['hostileContractTestDigest','test/contracts/r24-wp505-post-audit-compatibility.contract.test.mjs'],['testInventoryDigest','docs/OPS/R24/CORRECTIVE/C1B_TEST_INVENTORY_V1.json']])assert(successor.value.bindings[pathKey]===h(objectBytes(git,resolvedCandidate,artifactPath)),'E_WP505_SUCCESSOR_ARTIFACT_BINDING',artifactPath);
  assert(successor.value.evaluationPolicy?.wp505V1BaseSha===v1.baseSha&&successor.value.evaluationPolicy.wp505V1CandidateSha===v1.candidateSha&&successor.value.evaluationPolicy.wp505V2BaseSha===v2.baseSha&&successor.value.evaluationPolicy.v1ChangedPathDenominator===v1Changed.length&&successor.value.evaluationPolicy.v2ChangedPathDenominator===v2Changed.length&&successor.value.evaluationPolicy.arbitraryDescendantDeltaAccepted===false&&successor.value.historicalWp504BytesRewritten===false&&successor.value.productRuntimeBytesChangedBySuccessor===false&&successor.value.programDone===false,'E_WP505_SUCCESSOR_POLICY');
  for(const value of [failure.value.sourcePlanRoles,successor.value.sourcePlanRoles])assert(value?.externalSourcePlanDigest===EXTERNAL_SOURCE_PLAN_DIGEST&&value.compiledProgramFileDigest===COMPILED_PROGRAM_FILE_DIGEST&&value.rolesDistinct===true,'E_WP505_SUCCESSOR_SOURCE_ROLES');
  const admittedPaths=[...new Set([...v1.admittedPaths,...v2.admittedPaths])].sort();
  return{schemaVersion:'WP505_MAIN_PRODUCT_POST_EVALUATION_EXCEPTION_VERIFICATION_V1',status:'PASS',baseSha:v1.baseSha,v1CandidateSha:v1.candidateSha,candidateSha:resolvedCandidate,admissionDenominator:verified.length,v1ChangedPathDenominator:v1Changed.length,v2ChangedPathDenominator:v2Changed.length,admittedPathDenominator:admittedPaths.length,v1ChangedPaths:v1Changed,v2ChangedPaths:v2Changed,admittedPaths,admissions:verified.map(({authorityDigest,instanceDigest,admissionDigest,writeSetDigest,baseSha,baseTree})=>({authorityDigest,instanceDigest,admissionDigest,writeSetDigest,baseSha,baseTree})),failureDigest:failure.digest,successorDigest:successor.digest,sourcePlanRoles:{externalSourcePlanDigest:EXTERNAL_SOURCE_PLAN_DIGEST,compiledProgramFileDigest:COMPILED_PROGRAM_FILE_DIGEST,rolesDistinct:true}};
}
...
export function verifyCertificationSet({value,fileDigest,candidateSha='HEAD',git=defaultGit,allowAuditCycle2Admission=false,allowMainProductWp401Admission=false,allowMainProductWp402Admission=false,allowMainProductWp403Admission=false,allowMainProductWp404Admission=false,allowMainProductWp500Admission=false,allowMainProductWp501Admission=false,allowWp501GateIntegrationAdmission=false,allowWp501PerformanceIntegrationAdmission=false,allowWp501AuditR2CompatibilityAdmission=false,allowWp501InventoryFinalizationAdmission=false,allowWp501TerminalExceptionAdmission=false,allowMainProductWp502Admission=false,allowMainProductWp503Admission=false,allowMainProductWp504Admission=false,allowMainProductWp505Admission=false}){
  ...
  const wp505Enabled=allowMainProductWp505Admission||(allowAuditCycle2Admission&&wp505Descendant);
  ...
  const wp505Exception=wp505Enabled?verifyWp505MainProductPostEvaluationException({candidateSha:resolvedCandidate,git}):null;
  const allowedPaths=new Set([...ALLOWED_POST_EVALUATION_CARRIERS,...(cycle2Exception?.admittedPaths??[]),...(wp401Exception?.admittedPaths??[]),...(wp402Exception?.admittedPaths??[]),...(wp403Exception?.admittedPaths??[]),...(wp404Exception?.admittedPaths??[]),...(wp500Exception?.admittedPaths??[]),...(wp501Exception?.admittedPaths??[]),...(wp501GateException?.admittedPaths??[]),...(wp501PerformanceException?.admittedPaths??[]),...(wp501AuditR2Exception?.admittedPaths??[]),...(wp501InventoryException?.admittedPaths??[]),...(wp501TerminalException?.admittedPaths??[]),...(wp502Exception?.admittedPaths??[]),...(wp503Exception?.admittedPaths??[]),...(wp504Exception?.admittedPaths??[]),...(wp505Exception?.admittedPaths??[])]);
