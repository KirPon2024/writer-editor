...
const PATHS = Object.freeze({
  ...
  carrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V10.json`,
  predecessorCarrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V9.json`,
...
export function checkCorrections(root = process.cwd()) {
  ...
  assert(predecessorCarrierRegistry.digest === '4e0dccf984528fa798f74f41ebaf7acb8aa9ffcda3c09e76d32f7461b220e467' && predecessorCarrierRegistry.value.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V9', 'E_CARRIER_REGISTRY_PREDECESSOR_DIGEST', predecessorCarrierRegistry.digest);
  ...
  assert(carrierRegistry.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V10' && carrierRegistry.baseSha === stage.value.baseSha && carrierRegistry.baseTreeSha === stage.value.treeSha, 'E_CARRIER_REGISTRY_BINDING', carrierRegistry.schemaVersion);
  assertExactJson(carrierRegistry.predecessor,{path:PATHS.predecessorCarrierRegistry,sha256:predecessorCarrierRegistry.digest,status:'SUPERSEDED_BY_APPEND_ONLY_WP502_TEST_INVENTORY_SUCCESSOR'},'E_CARRIER_REGISTRY_PREDECESSOR','binding');
  ...
  assertExactJson(carrierRegistry.successorScope,{reason:'CURRENT_REPOSITORY_TEST_INVENTORY_EXTENDED_BY_ADMITTED_WP502_SINGLE_TERMINAL_PR',replacementPaths:[`${ROOT}/C1B_TEST_INVENTORY_V1.json`,'scripts/ops/r24/corrective/audit-r2-corrections.mjs','test/contracts/r24-audit-r2-corrections.contract.test.mjs'],addedPaths:['test/contracts/r24-wp502-terminal-carriers.contract.test.mjs','test/unit/r24-wp502-threads-causality-integration.test.js','test/unit/r24-wp502-threads-causality-mutants.test.js','test/unit/r24-wp502-threads-causality.test.js'],removedPaths:[]},'E_CARRIER_REGISTRY_SUCCESSOR_SCOPE','scope');
