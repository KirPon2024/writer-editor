...
const PATHS = Object.freeze({
  ...
  carrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V14.json`,
  predecessorCarrierRegistry: `${ROOT}/AUDIT_R2_CARRIER_REGISTRY_V13.json`,
...
export function checkCorrections(root = process.cwd()) {
  ...
  assert(predecessorCarrierRegistry.digest === 'c4391adfe540dfaac47857806dc4f97627506ba196d25dad78f3a91e9d27b168' && predecessorCarrierRegistry.value.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V13', 'E_CARRIER_REGISTRY_PREDECESSOR_DIGEST', predecessorCarrierRegistry.digest);
  ...
  assert(carrierRegistry.schemaVersion === 'AUDIT_R2_CARRIER_REGISTRY_V14' && carrierRegistry.baseSha === stage.value.baseSha && carrierRegistry.baseTreeSha === stage.value.treeSha, 'E_CARRIER_REGISTRY_BINDING', carrierRegistry.schemaVersion);
  assertExactJson(carrierRegistry.predecessor,{path:PATHS.predecessorCarrierRegistry,sha256:predecessorCarrierRegistry.digest,status:'SUPERSEDED_BY_APPEND_ONLY_WP503_WORDING_HASH_TEST_INVENTORY_SUCCESSOR'},'E_CARRIER_REGISTRY_PREDECESSOR','binding');
  ...
  assertExactJson(carrierRegistry.successorScope,{reason:'CURRENT_REPOSITORY_TEST_INVENTORY_REFRESHED_BY_ADMITTED_WP503_WORDING_HASH_SUCCESSOR',replacementPaths:[`${ROOT}/C1B_TEST_INVENTORY_V1.json`,'scripts/ops/r24/corrective/audit-r2-corrections.mjs','test/contracts/r24-audit-r2-corrections.contract.test.mjs'],addedPaths:[],removedPaths:[]},'E_CARRIER_REGISTRY_SUCCESSOR_SCOPE','scope');
