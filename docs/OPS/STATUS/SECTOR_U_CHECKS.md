...
| CHECK_U9_CLOSE_LOCK | 9 | `node --test test/unit/sector-u-close*.test.js` | exit code `0` when `SECTOR_U_PHASE=DONE` | `CHECK_PACK_FAIL` |
| SECTOR_U_FAST_02 | 10 | `node scripts/doctor.mjs` | `SECTOR_U_STATUS_OK=1` and `SECTOR_U_NO_RUNTIME_PRODUCT_WAIVERS_OK=1` and `SECTOR_U_FAST_DURATION_OK=1` and `U1_COMMANDS_PROOF_OK=1` and `U2_PROOF_OK=1` and `U2_TTL_EXPIRED=0` and `U3_EXPORT_PROOF_OK=1` and `U4_PROOF_OK=1` and `U5_PROOF_OK=1` and `SECTOR_U_CLOSE_OK=1` and `SECTOR_U_CLOSED_MUTATION=0` (for `SECTOR_U_PHASE=DONE`) | `DOCTOR_FAIL` |
...
| CHECK_U7_VISUAL_BASELINE | 2 | `node --test test/unit/sector-u-u7-*.test.js` with `SECTOR_U_FULL_VISUAL=1` | exit code `0` | `CHECK_PACK_FAIL` |
| CHECK_U8_PERF_BASELINE | 3 | `node --test test/unit/sector-u-u8-*.test.js` with `SECTOR_U_FULL_PERF=1` | exit code `0` | `CHECK_PACK_FAIL` |
| SECTOR_U_FULL_01 | 4 | `npm run test:sector-u-full` | exit code `0` | `TEST_FAIL` |
| SECTOR_U_FULL_02 | 5 | `node scripts/doctor.mjs` | `SECTOR_U_STATUS_OK=1` and `SECTOR_U_NO_RUNTIME_PRODUCT_WAIVERS_OK=1` | `DOCTOR_FAIL` |
