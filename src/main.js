...
function deriveReviewExactTextApplyOperationId(requestIdRaw, fallbackSeedRaw = '') {
  ...
  const fallbackSeed = typeof fallbackSeedRaw === 'string' ? fallbackSeedRaw.trim() : '';
...
async function readFullManuscriptDocxReviewPacketExportSource() {
  ...
  for (let index = 0; index < sceneCandidates.length; index += 1) {
    ...
    scenes.push({
      ...
      text: content.text,
      doc: content.doc,
      observableContent: content.observableContent,
      rawSha256: `sha256:${createRtkReviewTransportCryptoPort().sha256Text(content.observableContent)}`,
      order: index,
    });
  }
  const revisionBridge = await loadRevisionBridgeModule();
...
function getInternalCommandSurfaceKernel() {
  ...
  internalCommandSurfaceKernel = createCommandSurfaceKernel({
    ...
    [COMMAND_SURFACE_KERNEL_COMMAND_IDS.RTK_REVIEW_APPLY_MULTI_SCENE_FORMATTING_RETURN]: async (payload = {}) => {
      return handleRtkFormattingReturnCommandSurface(payload);
    },
  });
  return internalCommandSurfaceKernel;
}
...
function loadAtlasCurrentSceneDossierModule() {
  if (!atlasCurrentSceneDossierModulePromise) {
    ...
    atlasCurrentSceneDossierModulePromise = import(modulePath).catch((error) => {
      atlasCurrentSceneDossierModulePromise = null;
      throw error;
    });
  }
...
let atlasEntityDossierModulePromise = null;
function loadAtlasEntityDossierModule() {
  if (!atlasEntityDossierModulePromise) {
    const modulePath = pathToFileURL(path.join(__dirname, 'derived', 'atlas', 'deriveAtlasEntityDossier.mjs')).href;
    atlasEntityDossierModulePromise = import(modulePath).catch((error) => {
      atlasEntityDossierModulePromise = null;
      throw error;
    });
  }
...
let atlasMatricesModulePromise = null;
function loadAtlasMatricesModule() {
  if (!atlasMatricesModulePromise) {
    const modulePath = pathToFileURL(path.join(__dirname, 'derived', 'atlas', 'deriveAtlasMatrices.mjs')).href;
    atlasMatricesModulePromise = import(modulePath).catch((error) => {
      atlasMatricesModulePromise = null;
      throw error;
    });
  }
...
let atlasTemporalLayoutModulePromise = null;
function loadAtlasTemporalLayoutModule() {
  if (!atlasTemporalLayoutModulePromise) {
    const modulePath = pathToFileURL(path.join(__dirname, 'derived', 'atlas', 'deriveAtlasTemporalLayout.mjs')).href;
    atlasTemporalLayoutModulePromise = import(modulePath).catch((error) => {
      atlasTemporalLayoutModulePromise = null;
      throw error;
    });
  }
  return atlasTemporalLayoutModulePromise;
}

let atlasContinuityLedgerSurfaceModulePromise = null;
function loadAtlasContinuityLedgerSurfaceModule() {
  if (!atlasContinuityLedgerSurfaceModulePromise) {
    const modulePath = pathToFileURL(path.join(__dirname, 'derived', 'atlas', 'deriveAtlasContinuityLedgerSurface.mjs')).href;
    atlasContinuityLedgerSurfaceModulePromise = import(modulePath).catch((error) => {
      atlasContinuityLedgerSurfaceModulePromise = null;
      throw error;
    });
  }
...
function normalizeAtlasCurrentScenePayload(payload = {}) {
  const source = isPlainObjectValue(payload) ? payload : {};
  return {
    projectId: typeof source.projectId === 'string' ? source.projectId.trim() : '',
    nodeId: typeof source.nodeId === 'string' ? source.nodeId.trim() : '',
...
function makeAtlasCurrentSceneFallback(projectId, sceneId, reason, extra = {}) {
  return {
    schemaVersion: 'derived.atlas.currentSceneDossier.v1',
    ...
    surfaceManifest: {
      ...
      providerId: ATLAS_CURRENT_SCENE_QUERY_ID,
      host: 'rightRail',
    ...
    sceneTitle: typeof extra.sceneTitle === 'string' ? extra.sceneTitle : '',
    summary: { entityCount: 0, mentionCount: 0, sceneTextHash: '', indexHash: '', invalidationKey: '' },
    entities: [],
    mentions: [],
...
function makeAtlasEntityDossierFallback(projectId, entityId, reason, extra = {}) {
  return {
    schemaVersion: 'derived.atlas.entityDossier.v1',
    state: reason ? 'unavailable' : 'empty',
    unavailableReason: reason || '',
    surfaceManifest: {
      schemaVersion: 'surface.atlas.entityDossier.v1',
      surfaceId: 'surface.atlas.entityDossier',
      providerId: ATLAS_ENTITY_DOSSIER_QUERY_ID,
      host: 'rightRail',
    ...
    projectId: typeof projectId === 'string' ? projectId : '',
    requestedEntityId: typeof entityId === 'string' ? entityId : '',
    selectedEntityId: '',
    entity: null,
    aliases: [],
    summary: {
    ...
    absenceIntervals: [],
    evidenceLedger: {
      schemaVersion: 'derived.atlas.entityEvidenceLedger.v1',
      state: reason ? 'unavailable' : 'empty',
      readOnly: true,
      commandAuthority: 'none',
      rows: [],
...
function makeAtlasMatricesFallback(projectId, reason, extra = {}) {
  return {
    schemaVersion: 'derived.atlas.matrices.v1',
    state: reason ? 'unavailable' : 'empty',
    unavailableReason: reason || '',
    surfaceManifest: {
      schemaVersion: 'surface.atlas.matrices.v1',
      surfaceId: 'surface.atlas.matrices',
      providerId: ATLAS_MATRICES_QUERY_ID,
      host: 'rightRail',
    ...
    entitySceneMatrix: {
      schemaVersion: 'derived.atlas.entitySceneMatrix.v1',
      state: reason ? 'unavailable' : 'empty',
    ...
    relationMatrix: {
      schemaVersion: 'derived.atlas.relationMatrix.v1',
      state: reason ? 'unavailable' : 'empty',
      rowAxis: { kind: 'entity', totalCount: 0, visibleCount: 0, omittedCount: 0, clipped: false },
      columnAxis: { kind: 'entity', totalCount: 0, visibleCount: 0, omittedCount: 0, clipped: false },
      columns: [],
      rows: [],
    },
    listParity: {
    ...
    accessibilityContract: {
      schemaVersion: 'derived.atlas.matrixAccessibilityContract.v1',
      tableFirst: true,
...
function makeAtlasTemporalLayoutFallback(projectId, reason, extra = {}) {
  return {
    schemaVersion: 'derived.atlas.temporalLayout.v1',
    state: reason ? 'unavailable' : 'empty',
    unavailableReason: reason || '',
    surfaceManifest: {
      schemaVersion: 'surface.atlas.temporalLayout.v1',
      surfaceId: 'surface.atlas.temporalLayout',
      providerId: ATLAS_TEMPORAL_LAYOUT_QUERY_ID,
      host: 'rightRail',
    ...
    layoutPacket: {
      schemaVersion: 'derived.atlas.temporalLayoutPacket.v1',
      state: reason ? 'unavailable' : 'empty',
    ...
    timeSliderState: {
      schemaVersion: 'derived.atlas.timeSliderState.v1',
      min: 0,
      max: 0,
      step: 1,
      value: 0,
    ...
    listParity: {
      schemaVersion: 'derived.atlas.temporalLayoutListParity.v1',
      rows: [],
      equivalentToTimeline: true,
      omittedRowCount: 0,
    },
    keyboardContract: {
      schemaVersion: 'derived.atlas.temporalLayoutKeyboardContract.v1',
    ...
    largeProjectBudgetProof: {
      schemaVersion: 'derived.atlas.temporalLayoutBudgetProof.v1',
      sceneLimit: 0,
      segmentLimit: 0,
      totalSceneCount: 0,
      visibleSceneCount: 0,
...
function makeAtlasContinuityLedgerSurfaceFallback(projectId, reason, extra = {}) {
  return {
    schemaVersion: 'derived.atlas.continuityLedgerSurface.v1',
    state: reason ? 'unavailable' : 'empty',
    unavailableReason: reason || '',
    surfaceManifest: {
      schemaVersion: 'surface.atlas.continuityLedger.v1',
      surfaceId: 'surface.atlas.continuityLedger',
      providerId: ATLAS_CONTINUITY_LEDGER_SURFACE_QUERY_ID,
      host: 'rightRail',
    ...
    rows: [],
    listParity: {
      schemaVersion: 'derived.atlas.continuityLedgerListParity.v1',
      rows: [],
      equivalentToFindingRows: true,
      omittedRowCount: 0,
    },
    keyboardContract: {
      schemaVersion: 'derived.atlas.continuityLedgerKeyboardContract.v1',
...
async function buildAtlasCurrentSceneCoreState(resolvedNode, documentTarget) {
  ...
  if (!guard.ok || !guard.payload) {
    const error = new Error('PATH_BOUNDARY_VIOLATION');
  ...
  if (!parsed || parsed.issue) {
    const error = new Error('DOCUMENT_PAYLOAD_INVALID');
    error.code = 'E_ATLAS_CURRENT_SCENE_DOCUMENT_PAYLOAD_INVALID';
    throw error;
  }
  const context = getDocumentContextFromPath(filePath);
  return {
    ...
    coreState: {
      version: 1,
      data: {
        lastCommandId: Number.isInteger(resolvedNode.manifest?.lastCommandId)
          ? resolvedNode.manifest.lastCommandId
          : 0,
        projects: {
...
function normalizeAtlasEntityDossierPayload(payload = {}) {
  const source = isPlainObjectValue(payload) ? payload : {};
  return {
    projectId: typeof source.projectId === 'string' ? source.projectId.trim() : '',
    entityId: typeof source.entityId === 'string' ? source.entityId.trim() : '',
...
function normalizeAtlasMatricesPayload(payload = {}) {
  const source = isPlainObjectValue(payload) ? payload : {};
  return {
    projectId: typeof source.projectId === 'string' ? source.projectId.trim() : '',
...
function normalizeAtlasTemporalLayoutPayload(payload = {}) {
  const source = isPlainObjectValue(payload) ? payload : {};
  return {
    projectId: typeof source.projectId === 'string' ? source.projectId.trim() : '',
    explicitOpen: source.explicitOpen === true,
    sceneLimit: Number.isSafeInteger(Number(source.sceneLimit)) ? Number(source.sceneLimit) : 48,
    segmentLimit: Number.isSafeInteger(Number(source.segmentLimit)) ? Number(source.segmentLimit) : 32,
    sliderValue: Number.isFinite(Number(source.sliderValue)) ? Number(source.sliderValue) : null,
  };
}

function normalizeAtlasContinuityLedgerSurfacePayload(payload = {}) {
  const source = isPlainObjectValue(payload) ? payload : {};
  return {
    projectId: typeof source.projectId === 'string' ? source.projectId.trim() : '',
    explicitOpen: source.explicitOpen === true,
    rowLimit: Number.isSafeInteger(Number(source.rowLimit)) ? Number(source.rowLimit) : 16,
  };
}
...
async function handleWorkspaceAtlasEntityDossierQuery(payload = {}) {
  const safePayload = normalizeAtlasEntityDossierPayload(payload);
  const dossierModule = await loadAtlasEntityDossierModule();
  try {
    const { projectId, coreState } = await buildAtlasOverviewCoreState();
    if (safePayload.projectId && safePayload.projectId !== projectId) {
      return {
        ok: true,
        atlasEntityDossier: makeAtlasEntityDossierFallback(safePayload.projectId, safePayload.entityId, 'ATLAS_PROJECT_MISMATCH'),
      };
    }
    const dossier = dossierModule.deriveAtlasEntityDossier({
      coreState,
      params: {
        projectId,
        entityId: safePayload.entityId,
        limit: safePayload.limit,
      },
      capabilitySnapshot: {
        platformId: 'node',
    ...
    if (!dossier.ok) {
      return {
        ok: true,
        atlasEntityDossier: makeAtlasEntityDossierFallback(
          projectId,
          safePayload.entityId,
          dossier.error?.reason || 'ATLAS_ENTITY_DOSSIER_UNAVAILABLE',
    ...
    return {
      ok: true,
      atlasEntityDossier: dossier.value,
    };
  } catch (error) {
    logDevError('query.atlasEntityDossier', error);
    return {
      ok: true,
      atlasEntityDossier: makeAtlasEntityDossierFallback(
        safePayload.projectId,
...
async function handleWorkspaceAtlasMatricesQuery(payload = {}) {
  const safePayload = normalizeAtlasMatricesPayload(payload);
  const matricesModule = await loadAtlasMatricesModule();
  try {
    const { projectId, coreState } = await buildAtlasOverviewCoreState();
    if (safePayload.projectId && safePayload.projectId !== projectId) {
      return {
        ok: true,
        atlasMatrices: makeAtlasMatricesFallback(safePayload.projectId, 'ATLAS_PROJECT_MISMATCH'),
      };
    }
    const matrices = matricesModule.deriveAtlasMatrices({
      coreState,
      params: {
        projectId,
    ...
    if (!matrices.ok) {
      return {
        ...
        atlasMatrices: makeAtlasMatricesFallback(
          ...
          { error: matrices.error || null },
        ),
      };
    }
    return {
      ok: true,
      atlasMatrices: matrices.value,
    };
  } catch (error) {
    logDevError('query.atlasMatrices', error);
    return {
      ok: true,
      atlasMatrices: makeAtlasMatricesFallback(
        safePayload.projectId,
...
async function handleWorkspaceAtlasTemporalLayoutQuery(payload = {}) {
  const safePayload = normalizeAtlasTemporalLayoutPayload(payload);
  if (safePayload.explicitOpen !== true) {
    return {
      ok: true,
      atlasTemporalLayout: makeAtlasTemporalLayoutFallback(safePayload.projectId, 'ATLAS_TEMPORAL_LAYOUT_EXPLICIT_OPEN_REQUIRED'),
    };
  }
  const temporalLayoutModule = await loadAtlasTemporalLayoutModule();
  try {
    const { projectId, coreState } = await buildAtlasOverviewCoreState();
    if (safePayload.projectId && safePayload.projectId !== projectId) {
      return {
        ok: true,
        atlasTemporalLayout: makeAtlasTemporalLayoutFallback(safePayload.projectId, 'ATLAS_PROJECT_MISMATCH'),
      };
    }
    const temporalLayout = temporalLayoutModule.deriveAtlasTemporalLayout({
      coreState,
      params: {
        projectId,
        sceneLimit: safePayload.sceneLimit,
        segmentLimit: safePayload.segmentLimit,
        sliderValue: safePayload.sliderValue,
      },
      capabilitySnapshot: {
        platformId: 'node',
        capabilities: {
    ...
    if (!temporalLayout.ok) {
      return {
        ok: true,
        atlasTemporalLayout: makeAtlasTemporalLayoutFallback(
          projectId,
          temporalLayout.error?.reason || 'ATLAS_TEMPORAL_LAYOUT_UNAVAILABLE',
          { error: temporalLayout.error || null },
        ),
      };
    }
    return {
      ok: true,
      atlasTemporalLayout: temporalLayout.value,
    };
  } catch (error) {
    logDevError('query.atlasTemporalLayout', error);
    return {
      ok: true,
      atlasTemporalLayout: makeAtlasTemporalLayoutFallback(
        safePayload.projectId,
        error && typeof error.code === 'string' ? error.code : 'ATLAS_TEMPORAL_LAYOUT_READ_FAILED',
      ),
    };
  }
}

async function handleWorkspaceAtlasContinuityLedgerSurfaceQuery(payload = {}) {
  const safePayload = normalizeAtlasContinuityLedgerSurfacePayload(payload);
  if (safePayload.explicitOpen !== true) {
    return {
      ok: true,
      atlasContinuityLedgerSurface: makeAtlasContinuityLedgerSurfaceFallback(safePayload.projectId, 'ATLAS_CONTINUITY_LEDGER_EXPLICIT_OPEN_REQUIRED'),
    };
  }
  const continuityLedgerModule = await loadAtlasContinuityLedgerSurfaceModule();
  try {
    const { projectId, coreState } = await buildAtlasOverviewCoreState();
    if (safePayload.projectId && safePayload.projectId !== projectId) {
      return {
        ok: true,
        atlasContinuityLedgerSurface: makeAtlasContinuityLedgerSurfaceFallback(safePayload.projectId, 'ATLAS_PROJECT_MISMATCH'),
      };
    }
    const continuityLedger = continuityLedgerModule.deriveAtlasContinuityLedgerSurface({
      coreState,
      params: {
        projectId,
        rowLimit: safePayload.rowLimit,
    ...
    if (!continuityLedger.ok) {
      return {
        ok: true,
        atlasContinuityLedgerSurface: makeAtlasContinuityLedgerSurfaceFallback(
          projectId,
          continuityLedger.error?.reason || 'ATLAS_CONTINUITY_LEDGER_UNAVAILABLE',
          { error: continuityLedger.error || null },
        ),
      };
    }
    return {
    ...
    return {
      ...
      atlasContinuityLedgerSurface: makeAtlasContinuityLedgerSurfaceFallback(
        safePayload.projectId,
        error && typeof error.code === 'string' ? error.code : 'ATLAS_CONTINUITY_LEDGER_READ_FAILED',
      ),
    };
  }
}
...
async function handleWorkspaceAtlasCurrentSceneQuery(payload = {}) {
  const safePayload = normalizeAtlasCurrentScenePayload(payload);
  const dossierModule = await loadAtlasCurrentSceneDossierModule();
  if (!safePayload.nodeId) {
    return {
      ok: true,
      atlasCurrentScene: makeAtlasCurrentSceneFallback(safePayload.projectId, '', ''),
    };
  }
  ...
  try {
    ...
    return {
      ...
      atlasCurrentScene: makeAtlasCurrentSceneFallback(
        safePayload.projectId,
        safePayload.nodeId,
        error && typeof error.code === 'string' ? error.code : 'TREE_NODE_UNAVAILABLE',
      ),
    };
  }
  ...
  if (!ROMAN_CONTEXT_KINDS.has(documentTarget.kind)) {
    return {
      ...
      atlasCurrentScene: makeAtlasCurrentSceneFallback(
        resolvedNode.projectId,
        resolvedNode.nodeId,
        'ATLAS_UNSUPPORTED_FOR_NODE',
      ),
    };
  }

  try {
    const { coreState, sceneTitle } = await buildAtlasCurrentSceneCoreState(resolvedNode, documentTarget);
    const dossier = dossierModule.deriveAtlasCurrentSceneDossier({
      coreState,
      params: {
        projectId: resolvedNode.projectId,
    ...
    if (!dossier.ok) {
      return {
        ...
        atlasCurrentScene: makeAtlasCurrentSceneFallback(
          resolvedNode.projectId,
          resolvedNode.nodeId,
          dossier.error?.reason || 'ATLAS_DOSSIER_UNAVAILABLE',
          { sceneTitle, error: dossier.error || null },
        ),
      };
    }
    return {
      ok: true,
      atlasCurrentScene: dossier.value,
    };
  } catch (error) {
    logDevError('query.atlasCurrentScene', error);
    return {
      ok: true,
      atlasCurrentScene: makeAtlasCurrentSceneFallback(
        resolvedNode.projectId,
        resolvedNode.nodeId,
        error && typeof error.code === 'string' ? error.code : 'ATLAS_DOSSIER_READ_FAILED',
      ),
    };
  }
}
...
let rtkFormattingReturnModulePromise = null;
function loadRtkFormattingReturnModule() {
  if (!rtkFormattingReturnModulePromise) {
    const modulePath = pathToFileURL(path.join(
      __dirname,
      'io',
      'revisionBridge',
    ...
    rtkFormattingReturnModulePromise = import(modulePath).catch((error) => {
      rtkFormattingReturnModulePromise = null;
      throw error;
    });
  }
  return rtkFormattingReturnModulePromise;
}

async function handleRtkFormattingReturnCommandSurface(payload = {}) {
  let module = null;
  try {
    module = await loadRtkFormattingReturnModule();
  } catch (error) {
    return makeReviewMutateTypedError(
      'cmd.rtk.review.applyMultiSceneFormattingReturn',
      'E_RTK_FORMATTING_RETURN_UNAVAILABLE',
      'RTK_FORMATTING_RETURN_UNAVAILABLE',
      { message: error && typeof error.message === 'string' ? error.message : 'UNKNOWN' },
    );
  }
  if (!module || typeof module.createRtkFormattingReturnCommandHandler !== 'function') {
    return makeReviewMutateTypedError(
      'cmd.rtk.review.applyMultiSceneFormattingReturn',
      'E_RTK_FORMATTING_RETURN_UNAVAILABLE',
...
async function reconcileReviewFormattingReturnAtStartup() {
  ...
  if (!result || result.ok !== true) {
    ...
    error.code = docxReviewPreviewSessionDetailString(result?.code)
      || 'RTK_FORMATTING_STARTUP_RECOVERY_FAILED';
    throw error;
  }
...
app.whenReady().then(async () => {
  ...
  if (appInitializationError) {
    dialog.showErrorBox(
      'Yalken recovery required',
      `Project recovery failed safely: ${appInitializationError.code || appInitializationError.message || 'UNKNOWN'}`,
    );
    app.quit();
    return;
  }
