...
const MUTANTS = Object.freeze([
  ...
  },
  {
    ...
    find: "    if (row.headSha !== expectedHeadSha) return fail('E_R24_V1_GATE_HEAD_MISMATCH', `${stageId}:${row.headSha} != ${expectedHeadSha}`);",
    replace: "    if (false) return fail('E_R24_V1_GATE_HEAD_MISMATCH', `${stageId}:${row.headSha} != ${expectedHeadSha}`);",
    oracle: async (c) => {
      const input = validInput(c);
      input.gateEvidence = c.buildGateEvidenceFromWorkflowPrefix({
        program: input.program,
