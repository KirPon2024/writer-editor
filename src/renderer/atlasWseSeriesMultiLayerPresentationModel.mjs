const VIEW_LABELS = Object.freeze({
  seriesCanon: 'Series canon',
  multiLayerAtlas: 'Multi-layer atlas',
  evidenceCapsule: 'Evidence capsule',
  agentContextPacket: 'Agent context',
});
...
export function normalizeWseSeriesMultiLayerPresentation(source = {}, requestedView = 'seriesCanon') {
  ...
  const rows = Array.isArray(selected.rows)
    ...
    : [];
  return {
    schemaVersion: typeof projection.schemaVersion === 'string' ? projection.schemaVersion : 'yalken.r24.wseSeriesMultiLayer.v1',
    ...
    seriesId: typeof projection.seriesId === 'string' ? projection.seriesId : '',
    viewId,
    ...
    privacy: {
      metadataOnly: object(projection.privacy).metadataOnly === true,
      sourceContentIncluded: object(projection.privacy).sourceContentIncluded === true,
      privateOwnerDataIncluded: object(projection.privacy).privateOwnerDataIncluded === true,
      pathsIncluded: object(projection.privacy).pathsIncluded === true,
      secretsIncluded: object(projection.privacy).secretsIncluded === true,
    },
