...
function validate(values, decision = values.decision, digest = values.decisionDigest) {
  return validateOwnerGateAmendments({
    ...
    loadDecisionArtifact: () => ({ value: decision, digest }),
  });
}

test('exact owner decision yields only the mission-bound R2 storage approval', () => {
  const values = fixture();
  assert.equal(values.registry.entries.find((entry) => entry.id === 'STORAGE_AUTHORITY_ADR').status, 'UNRESOLVED');
  assert.deepEqual(validate(values), { STORAGE_AUTHORITY_ADR: 'APPROVED' });
});
...
test('wrong mission digest is rejected even if a mutant is resealed', () => {
  ...
  assert.throws(() => validate(values, decision), (error) => error.code === 'E_R24_OWNER_GATE_DECISION_MISSION');
});
...
test('wrong node or gate cannot inherit the R2 decision', () => {
  ...
  assert.throws(() => validate(values, wrongNode), (error) => error.code === 'E_R24_OWNER_GATE_DECISION_NODE');
...
test('artifact byte drift fails before semantic approval', () => {
  const values = fixture();
  assert.throws(
    () => validate(values, values.decision, 'f'.repeat(64)),
