...
function runDoctorForClose({ statusPath, reportPath, lockPath }) {
...
test('doctor marks sector-m close tokens ready/ok with valid lock', () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sector-m-close-'));
  ...
  assert.deepEqual(JSON.parse(tokens.get('SECTOR_M_CLOSED_MUTATION_VIOLATIONS')), []);
});

test('doctor marks sector-m close invalid when report/lock are missing', () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sector-m-close-invalid-'));
  const statusPath = path.join(tmpRoot, 'SECTOR_M.json');
  const reportPath = path.join(tmpRoot, 'SECTOR_M_CLOSE_REPORT.md');
  const lockPath = path.join(tmpRoot, 'SECTOR_M_CLOSED_LOCK.json');
  ...
  assert.ok(violations.includes('CLOSE_REPORT_MISSING'));
});
