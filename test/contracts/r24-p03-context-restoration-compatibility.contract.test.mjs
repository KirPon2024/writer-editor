...
test('context contract schema protocol and current CI retain no-authority and full fallback laws',()=>{
 ...
 assert(workflow.indexOf('Reject stale context caches')<workflow.indexOf('- run: npm ci --engine-strict'));assert(workflow.includes('run: npm run r24:test-inventory'));assert.equal(historicalWorkflow.split('P03_GOVERNANCE_CHANGE_APPROVALS').length,4);assert.equal(workflow.split('WP603_PACKAGED_RECOVERY_P03_COMPATIBILITY_ROUTING_GOVERNANCE_CHANGE_APPROVALS').length,4);
