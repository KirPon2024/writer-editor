...
test('the append-only audit-R2 carrier chain passes the static exact-byte check',()=>{
  ...
  assert.equal(result.schemaVersion,'AUDIT_R2_CORRECTION_STATIC_CHECK_V7');
  assert.equal(result.predecessorCarrierRegistryDigest,'3912c79ac74885f56c56688bd5aa66b5267b6f1213d3ec487081b58a9865db05');
...
test('audit-R2 registry successor binds distinct source roles and only replaces the current WP501 inventory entry',()=>{
  const value=load('docs/OPS/R24/CORRECTIVE/AUDIT_R2_CARRIER_REGISTRY_V7.json');
  assert.equal(value.predecessor.sha256,'3912c79ac74885f56c56688bd5aa66b5267b6f1213d3ec487081b58a9865db05');
  assert.equal(value.predecessor.status,'SUPERSEDED_BY_APPEND_ONLY_WP501_TEST_INVENTORY_SUCCESSOR');
