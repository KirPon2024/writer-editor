...
function fixture(mutator=()=>{}) {
  ...
  const instanceFile=raw('docs/OPS/R24/CORRECTIVE/POST_AUDIT_CORRECTIONS_STAGE_INSTANCE_V10.json');
  const admissionFile=raw('docs/OPS/R24/CORRECTIVE/POST_AUDIT_CORRECTIONS_STAGE_ADMISSION_ATTESTATION_V10.json');
...
test('protected workflow is stage-specific and binds current V10 closure authority plus exact one-file artifact',()=>{const workflow=fs.readFileSync('.github/workflows/r24-terminal-attestation.yml','utf8');for(const token of ['test "$STAGE_ID" = POST_AUDIT_CORRECTIONS','POST_AUDIT_CORRECTIONS_STAGE_INSTANCE_V10.json','POST_AUDIT_CORRECTIONS_STAGE_ADMISSION_ATTESTATION_V10.json','E_CANDIDATE_SECOND_PARENT','E_CLOSURE_STATE_PREDECESSOR','E_CLOSURE_RECEIPT_PREDECESSOR','test "$(find "$RUNNER_TEMP/r24-terminal-attestation" -type f | wc -l | tr -d \' \')" = 1','name: r24-terminal-attestation-${{ inputs.stage_id }}'])assert.equal(workflow.includes(token),true,token);assert.equal(workflow.includes('wp400MutationStarted:false'),false);});
