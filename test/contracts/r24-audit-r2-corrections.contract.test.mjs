...
test('the append-only audit-R2 carrier chain passes the static exact-byte check',()=>{
  ...
  assert.equal(result.schemaVersion,'AUDIT_R2_CORRECTION_STATIC_CHECK_V8');
  assert.equal(result.predecessorCarrierRegistryDigest,'0ac29f5bd4608a819738b43f3417aca49923d78d24c1119c1239e951330d43a3');
...
test('audit-R2 registry successor binds distinct source roles and only replaces the final WP501 inventory entry',()=>{
  const value=load('docs/OPS/R24/CORRECTIVE/AUDIT_R2_CARRIER_REGISTRY_V8.json');
  assert.equal(value.predecessor.sha256,'0ac29f5bd4608a819738b43f3417aca49923d78d24c1119c1239e951330d43a3');
  assert.equal(value.predecessor.status,'SUPERSEDED_BY_APPEND_ONLY_WP501_FINAL_TEST_INVENTORY_SUCCESSOR');
